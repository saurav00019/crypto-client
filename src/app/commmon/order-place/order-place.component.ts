...
export class OrderPlaceComponent implements OnInit {
  ...
  constructor( private web2:Web2Service,private shared: SharedDataService,private fb: UntypedFormBuilder,private toaster: ToastrService, private web:WebsocketService){
    // console.log("order itme1", this.items1)
    // 62.216.82.94
    //  http://62.216.82.94:6102/CryptoExch
    //  ws://62.216.82.94:9699
    //  ws://62.216.82.94:9697
    // https://apibitz.bitziana.com/Tradersroom_API_bitziana
    // this.socket = new WebSocket('wss://apitest.bitziana.com:9797');
    this.socket = new WebSocket('wss://apibitz.bitziana.com:9797');
    // this.socket = new WebSocket('ws://62.216.82.94:9697');
  ...
  ngOnInit()
{
  ...
  ngOnChanges(changes: SimpleChanges) {
   ...
   this.currentPrice= this.placeOrderData?.oQuote?.L
  ...
  placeOrderLimit(val: any,orderType: any){
    ...
    if(this.makeOrdersLimit.value.sl == ""){
      ...
      let obj ={
        ID:105,
        Key:1,
        SymID:Number(this.placeOrderData?.oQuote?.ID),
     ...
     console.log("obbjjjjj",obj);
      ...
      // this.shared.addDataToSharedArray(obj);
...
this.shared.obSym(1)
  ...
  this.shared.addDataToSharedArray1(originalData1);
  this.shared.getOrderCanRes(originalData1)
