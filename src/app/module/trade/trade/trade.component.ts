...
export class TradeComponent implements OnInit {
  ...
  GateWayId: any;
  makePayment: any;
  constructor( private modalService: NgbModal, private sharedData:SharedDataService, private api : ApiDataService ,
     private toastrService: ToastrService,private http: HttpClient,  private fb: UntypedFormBuilder,
  ...
  this.sharedData.loader(true)
    this.api.getTrade(obj).subscribe({
      ...
      if(val === 3){
         ...
         this.tradeListLength =this.tradeList.length
        //  this.dateFilter= this.tradeList[4].oTradeTime.sCreadedOn_Str
        //  console.log("her is date filter", this.dateFilter);

        // //  this.convertedDate = this.datePipe.transform(this.dateFilter, 'yyyy-MM-dd HH:mm:ss', ' UTC');
        //  this.convertedDate = new Date(this.dateFilter + ' UTC');
        //  this.trimmedDate = this.datePipe.transform(this.convertedDate, 'EEE MMM dd yyyy hh:mm:ss');
        //  console.log(" here is correct date with date pipe>>>>>>>>>>>>>>>>>", this.convertedDate);
        //  console.log(" here is correct date with date pipe>>>>>>>>>>>>>>>>>", this.trimmedDate);
