...
this.step = 1
    this.jsonBytes = []
    this.jsonBytes = this.orderSend(105, 1, 101, 1070, 1, 2, 18, 15, 10.3, this.alpha);
  ...
  orderSend(ID: number, Key: number, SymID: number, U: number, Buy_Sell: number, T: number, P: number, L: number, SL: number, C: string): Uint8Array {
    ...
    view.setUint16(6, SymID, true);
    view.setUint16(8, U, true);
    view.setUint16(10, Buy_Sell, true);
    view.setUint8(12, T);
    view.setFloat64(14, P, true);
    view.setFloat64(22, L, true);
    view.setFloat64(30, SL, true);

    // Convert string to an array of UTF-8 encoded bytes
    const stringBytes = new TextEncoder().encode(C);
...
const orderNo = new TextDecoder().decode(new Uint8Array(stringBytes1)).trim();
 ...
 console.log(" this.OrderNooooo", this.OrderNooooo)
...
orderDecrypt(byteArray: Uint8Array): any {
  ...
  const Buy_Sell = view.getUint16(10, true);
  const T = view.getUint8(12);
  const P = view.getFloat64(14, true);
  
  const L = view.getFloat64(22, true);
  const SL = view.getFloat64(30, true);

  // Decode the string from UTF-8 encoded bytes
  let stringBytes = [];
  for (let i = 30; i < byteArray.length; i++) {
    stringBytes.push(view.getUint8(i));
  }
  const C = new TextDecoder().decode(new Uint8Array(stringBytes));
...
Order1() {
  ...
  this.step = 4
      this.jsonBytes = []
      this.jsonBytes = this.orderSend1(105, 1, 101, 1070, 2, 2, 18, 15, 10.3,this.alpha);
    ...
    orderSend1(ID: number, Key: number, SymID: number, U: number, Buy_Sell: number, T: number, P: number, L: number, SL: number, C: string): Uint8Array {
      const buffer = new ArrayBuffer(48); // Total byte length based on your specified lengths
      const view = new DataView(buffer);
      ...
      const stringBytes = new TextEncoder().encode(C);
      for (let i = 0; i < stringBytes.length; i++) {
        view.setUint8(30 + i, stringBytes[i]);
      }
...
OrderCancel() {
  ...
  this.step = 2
      this.jsonBytes = []
      this.jsonBytes = this.ordeCancel(107,1, 1070, 18,15,this.OrderNooooo);
    ...
    ordeCancel(ID: number, Key: number, UserID: number, Price: number, Lot: number,OrdNo:string ): Uint8Array {
...
OrderModify() {
  debugger
      this.jsonBytes = []
      this.jsonBytes = this.orderMod(106,2,3,18.2,3.3,19,16,'ABCD');
    ...
    orderMod(ID: number, Key: number, UserID: number, SL: number, TP: number, Price: number, Lot: number,OrdNo:string ): Uint8Array {
      ...
      view.setUint32(2, Key, true);
      view.setUint16(6, UserID, true);
      view.setFloat64(8, SL, true);
      view.setFloat64(16, TP, true);
    
     
    
      view.setFloat64(24, Price, true);
      view.setFloat64(32, Lot, true);
